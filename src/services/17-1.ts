...
async function readFileExample(): Promise<string> {
  try {
    const data = await fs.readFile("src/utils/2017-1.md", "utf8");
    return data;
  } catch (err) {
...
(async () => {
  ...
  const dia = apresentacao.map((v) => {
    ...
    // Para 2017-1: formato "2a feira, 10/07/2017, 14:00hs, Local"
    const diaMatch = v.match(/(\d{1,2}\/\d{1,2}\/\d{4})/);
    if (diaMatch) {
      return diaMatch[1]; // Retorna "10/07/2017"
    }

    // Fallback para outros formatos
    const diaFallback = v.slice(v.indexOf("dia: ") + 5, v.indexOf(","));
    if (diaFallback && diaFallback.trim() !== "DD/MM/AAAA") {
      return diaFallback.trim();
    }
