...
async function readFileExample(): Promise<string> {
  try {
    const data = await fs.readFile("src/utils/2021-1.md", "utf8");
    return data;
  } catch (err) {
...
function mapFieldsFromRaw(newRaw2: any[]) {
  ...
  return {
    ...
    apresentacao: newRaw2.map((v) => getFieldWithValidation(v, ["defesa"])),

    banca: newRaw2.map((v) => getFieldWithValidation(v, ["banca"])),

    date: newRaw2.map((v) => getFieldWithValidation(v, ["data"])),

    horaLocal: newRaw2.map((v) => getFieldWithValidation(v, ["local"])),
  };
}
...
(async () => {
  ...
  const local = horaLocal.map((v, index) => {
    return horaLocal[index];
  });
