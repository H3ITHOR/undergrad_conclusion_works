...
(async () => {
  ...
  const bigString = await page.evaluate(() => {
    ...
    // const siIndex = allElements.findIndex((el) =>
    //   el.textContent.includes("Sistemas de Informação")
    // );

    // const elementsAfterSI = allElements.slice(siIndex + 1);

    // // return elementsAfterSI.map((el) => el.textContent || "").join("\n");
    // return allElements;
  });

  const elementsArray = await page.evaluate(() => {
    const ol = document.querySelector("body");

    const SIPage = ol.querySelectorAll("b");

    const listItems = Array.from(ol.querySelectorAll("b"));
    const SIlistItems = listItems.slice(
      listItems.findIndex((el) => el.innerText === "Sistemas de Informação") + 1
    );

    // const raw = SIlistItems.map(v => )
    console.log(SIlistItems.map((el) => el.innerText));
    return SIlistItems.map((el) => el.textContent.replace(/\n/g, " ").trim());
  });

  const HTMLelementsArray = await page.evaluate(() => {
    const ol = document.querySelector("body > b");
